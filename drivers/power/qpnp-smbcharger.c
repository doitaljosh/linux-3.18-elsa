...
static int get_usb_adc(struct smbchg_chip *chip)
{
	...
	if (IS_ERR_OR_NULL(chip->vadc_usbin_dev)) {
		...
		if (IS_ERR_OR_NULL(chip->vadc_usbin_dev)) {
			pr_dbg(PR_LGE, "vadc is not init yet\n");
...
static int lgcc_set_charging_enable(struct smbchg_chip *chip,
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to set Charging Status \n");
...
static int batt_life_cycle_set_fcc_ma (struct smbchg_chip *chip) {
    ...
    if (MAX_CYCLE_STEP == i) {
        pr_dbg(PR_LGE, "now DECCUR state %d \n", chip->vfloat_mv);
        return rc;
    }

    pr_dbg(PR_LGE, "vfloat %d fcc-ma %d",
									...
									u8 val)
{
	...
	pr_dbg(PR_REGISTER, "addr = 0x%x writing 0x%x\n", base, reg);
...
static void smbchg_stay_awake(struct smbchg_chip *chip, int reason)
{
	...
	if (reasons != 0 && chip->wake_reasons == 0) {
		pr_dbg(PR_PM, "staying awake: 0x%02x (bit %d)\n",
				reasons, reason);
...
static void smbchg_relax(struct smbchg_chip *chip, int reason)
{
	...
	reasons = chip->wake_reasons & (~reason);
	if (reasons == 0 && chip->wake_reasons != 0) {
		pr_dbg(PR_PM, "relaxing: 0x%02x (bit %d)\n",
				reasons, reason);
		pm_relax(chip->dev);
	}
	chip->wake_reasons = reasons;
...
static bool is_otg_present_schg(struct smbchg_chip *chip)
{
	...
	if ((reg & FMB_STS_MASK) != 0) {
		pr_dbg(PR_STATUS, "IDEV_STS = %02x, not ground\n", reg);
		return false;
	}
	...
	if (usbid_val > USBID_GND_THRESHOLD) {
		pr_dbg(PR_STATUS, "USBID = 0x%04x, too high to be ground\n",
				usbid_val);
		return false;
	}
...
out:
	pr_dbg_rt(PR_MISC, "CHGR_STS = 0x%02x\n", reg);
	return status;
}
	...
	enum power_supply_property prop, int val)
{
	...
	if (!chip->bm_psy) {
		pr_dbg(PR_STATUS, "no bm psy found\n");
		return -EINVAL;
	}
		...
		enum power_supply_property prop, int val)
{
	...
	if (!chip->bms_psy) {
		pr_dbg(PR_STATUS, "no bms psy found\n");
		return -EINVAL;
	}
...
static int get_prop_batt_capacity(struct smbchg_chip *chip)
{
	...
	if (rc) {
		pr_dbg(PR_STATUS, "Couldn't get capacity rc = %d\n", rc);
...
static int get_prop_batt_temp(struct smbchg_chip *chip)
{
	...
	if (rc) {
		pr_dbg(PR_STATUS, "Couldn't get temperature rc = %d\n", rc);
...
static int get_prop_batt_current_now(struct smbchg_chip *chip)
{
	...
	if (rc) {
		pr_dbg(PR_STATUS, "Couldn't get current rc = %d\n", rc);
...
static int get_prop_batt_resistance_id(struct smbchg_chip *chip)
{
	...
	rc = get_property_from_fg(chip, POWER_SUPPLY_PROP_RESISTANCE_ID,
			&rbatt);
	if (rc) {
		pr_dbg(PR_STATUS, "Couldn't get resistance id rc = %d\n", rc);
...
static int get_prop_batt_full_charge(struct smbchg_chip *chip)
{
	...
	if (rc) {
		pr_dbg(PR_STATUS, "Couldn't get charge_full rc = %d\n", rc);
...
static int get_prop_batt_voltage_now(struct smbchg_chip *chip)
{
	...
	if (rc) {
		pr_dbg(PR_STATUS, "Couldn't get voltage rc = %d\n", rc);
...
static int get_prop_battery_condition(struct smbchg_chip *chip)
{
	...
	if (rc) {
		pr_dbg(PR_STATUS, "Couldn't get age rc = %d\n", rc);
				...
				union power_supply_propval *prop)
{
	...
	if (!chip->typec_psy) {
		...
		if (!chip->typec_psy) {
			pr_dbg(PR_LGE, "typec psy doesn't prepared\n");
			prop->intval = 0;
			return;
		}
	}
		...
		if (!chip->skip_usb_notification) {
			...
			if (rc)
				pr_err("typec failed to set current max rc=%d\n",
					rc);
			pr_dbg(PR_TYPEC, "SMB Type-C mode = %d, current=%d\n",
...
static int smbchg_set_dc_current_max(struct smbchg_chip *chip, int current_ma)
{
	...
	pr_dbg(PR_STATUS, "dc current set to %d mA\n",
								...
								int period_s)
{
	...
	pr_dbg(PR_STATUS, "aicl rerun period set to %ds\n",
...
static struct power_supply *get_parallel_psy(struct smbchg_chip *chip)
{
	...
	if (!chip->parallel.psy)
		pr_dbg(PR_STATUS, "parallel charger not found\n");
	return chip->parallel.psy;
}
...
static void smbchg_usb_update_online_work(struct work_struct *work)
{
	...
	if (chip->usb_psy && chip->usb_psy->get_property &&
		...
		if (!!val.intval && !online) {
			pr_dbg(PR_LGE, "Updating by force\n");
	...
	if (chip->usb_online != online) {
		pr_dbg(PR_MISC, "setting usb psy online = %d\n", online);
							...
							int current_ma)
{
	...
	pr_dbg(PR_STATUS, "USB current_ma = %d\n", current_ma);
...
out:
	pr_dbg(PR_STATUS, "usb type = %d current set to %d mA\n",
...
static bool is_hvdcp_present(struct smbchg_chip *chip)
{
	...
	pr_dbg(PR_STATUS, "HVDCP_STS = 0x%02x\n", reg);
	/*
							...
							int current_ma)
{
	...
	if (chip->tables.usb_ilim_ma_table[i] == chip->fastchg_current_ma) {
		pr_dbg(PR_STATUS, "skipping fastchg current request: %d\n",
	...
	pr_dbg(PR_STATUS, "fastcharge current requested %d, set to %d\n",
...
static int smbchg_sw_esr_pulse_en(struct smbchg_chip *chip, bool en)
{
	...
	rc = get_property_from_fg(chip, POWER_SUPPLY_PROP_CURRENT_NOW,
						&fg_current_now);
	if (rc) {
		pr_dbg(PR_STATUS, "bms psy does not support OCV\n");
		return 0;
	}
...
static void smbchg_rerun_aicl(struct smbchg_chip *chip)
{
	...
	pr_dbg(PR_STATUS, "Rerunning AICL...\n");
...
static int smbchg_run_parallel_aicl(struct smbchg_chip *chip)
{
	...
	pr_dbg(PR_STATUS, "parallel AICL = %d mA\n",
...
static void smbchg_parallel_usb_disable(struct smbchg_chip *chip)
{
	...
	if (!parallel_psy || !chip->parallel_charger_detected)
		return;
	pr_dbg(PR_STATUS, "disabling parallel charger\n");
...
try_again:
	...
	if (chip->parallel.current_max_ma == 0) {
		pr_dbg(PR_STATUS, "Not parallel charging, skipping\n");
		goto done;
	}
	...
	pr_dbg(PR_STATUS, "try #%d parallel charger fcc = %d\n",
...
#endif
	pr_dbg(PR_STATUS, "reducing FCC of parallel charger to %d\n",
		pval.intval);
	/* Change it to uA */
	pval.intval *= 1000;
...
static void smbchg_parallel_usb_taper_work(struct work_struct *work)
{
	...
	if (chip->parallel.current_max_ma == 0) {
		pr_dbg(PR_STATUS, "Not parallel charging, unvote\n");
	...
	pr_dbg(PR_LGE, "TOTAL IBAT[%d (%d + %d)] vote for split current\n",
			fastchg_current_ma,
...
static void smbchg_battchg_protect_work(struct work_struct *work)
{
...
#endif
			pr_dbg(PR_LGE, "battery voltage too low, unvote %s\n",
					TAPER_FCC_VOTER);
			vote(chip->fcc_votable, TAPER_FCC_VOTER, false, 0);
	...
	pr_dbg(PR_LGE, "battery voltage reached threshold (%d > %d), "\
			"reducing fcc to %d\n",
			vbatt,
#ifdef CONFIG_LGE_PM_CYCLE_BASED_CHG_VOLTAGE
		...
		int total_current_ma)
{
	...
	pr_dbg(PR_STATUS, "Attempting to enable parallel charger\n");
	...
	pr_dbg(PR_LGE, "New main ICL percent = %d\n", smbchg_main_chg_icl_percent);
#endif
	...
	pr_dbg(PR_MISC, "Requested ICL = %d from parallel, got %d\n",
		new_parallel_cl_ma, set_parallel_cl_ma);
	new_pmi_cl_ma = max(0, target_icl_ma - set_parallel_cl_ma);
	pr_dbg(PR_STATUS, "New Total USB current = %d[%d, %d]\n",
	...
	pr_dbg(PR_MISC, "Requested FCC = %d from parallel, got %d\n",
		...
		int *ret_total_current_ma)
{
	...
	if (!parallel_psy || !smbchg_parallel_en
			|| !chip->parallel_charger_detected) {
		pr_dbg(PR_STATUS, "Parallel charging not enabled\n");
		return false;
	}
	...
	if (chip->parallel.current_max_ma == 0
		...
		pr_dbg(PR_STATUS, "Only been %lld since disable, skipping\n",
				ktime_to_ms(kt_since_last_disable));
		return false;
	}
	...
	if (get_prop_charge_type(chip) != POWER_SUPPLY_CHARGE_TYPE_FAST
			&& (get_prop_batt_present(chip)
				|| chip->parallel.current_max_ma == 0)) {
		pr_dbg(PR_STATUS, "Not in fast charge, skipping\n");
		return false;
	}

	if (get_prop_batt_health(chip) != POWER_SUPPLY_HEALTH_GOOD) {
		pr_dbg(PR_STATUS, "JEITA active, skipping\n");
		return false;
	}
	...
	if (chip->typec_psy &&
		...
		pr_dbg(PR_LGE, "typec_current_ma is %d, continuing\n",
	...
	if (get_usb_supply_type(type) == POWER_SUPPLY_TYPE_USB_CDP) {
		pr_dbg(PR_STATUS, "CDP adapter, skipping\n");
		return false;
	}

	if (get_usb_supply_type(type) == POWER_SUPPLY_TYPE_USB) {
		pr_dbg(PR_STATUS, "SDP adapter, skipping\n");
		return false;
	}
	...
	if (chip->usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP &&
		...
		pr_dbg(PR_LGE, "DCP adapter is weak, skipping\n");
		return false;
	}
#endif
...
skip_usb_supply_type:
	...
	if (!smbchg_is_usbin_active_pwr_src(chip)) {
		pr_dbg(PR_STATUS, "USB not active power source: %02x\n", reg);
		return false;
	}

	min_current_thr_ma = smbchg_get_min_parallel_current_ma(chip);
	if (min_current_thr_ma <= 0) {
		pr_dbg(PR_STATUS, "parallel charger unavailable for thr: %d\n",
				min_current_thr_ma);
		return false;
	}

	if (usb_icl_ma < min_current_thr_ma) {
		pr_dbg(PR_STATUS, "Weak USB chg skip enable: %d < %d\n",
	...
	if ((!fcc_voter || strcmp(fcc_voter, ESR_PULSE_FCC_VOTER) != 0)
		...
		pr_dbg(PR_STATUS, "FCC %d lower than %d\n",
			fcc_ma,
	...
	if (chip->parallel.initial_aicl_ma == 0) {
		if (current_limit_ma < min_current_thr_ma) {
			pr_dbg(PR_STATUS, "Initial AICL very low: %d < %d\n",
	...
	if (chip->parallel.use_parallel_aicl) {
		...
		pr_dbg(PR_STATUS, "use parallel AICL result: %dmA\n",
						total_current_ma);
...
static void smbchg_parallel_usb_en_work(struct work_struct *work)
{
	...
	if (chip->parallel.use_parallel_aicl) {
		while (tries--) {
			...
			if (rc >= 0) {
				pr_dbg(PR_STATUS, "hvdcp detection done\n");
				break;
			}
			pr_dbg(PR_STATUS, "wait hvdcp_det_done interrupted\n");
		}
	}
	...
	if (smbchg_is_parallel_usb_ok(chip, &total_current_ma)) {
		...
		if (in_progress) {
			pr_dbg(PR_STATUS, "parallel charging unavailable\n");
			smbchg_parallel_usb_disable(chip);
		}
	}
	mutex_unlock(&chip->parallel.lock);
	pr_dbg(PR_STATUS, "Parallel check complete!\n");
...
static void smbchg_parallel_usb_check_ok(struct smbchg_chip *chip)
{
	...
	if (chip->parallel.parallel_en_in_progress) {
		pr_dbg(PR_MISC, "parallel logic run in progress, ignore\n");
		return;
	}
...
#ifdef CONFIG_LGE_PM_PARALLEL_CHARGING
	pr_dbg(PR_LGE, "client[%s], suspend[%d]\n",client,suspend);
#endif
							...
							int current_ma)
{
	...
	pr_dbg(PR_STATUS, "User setting FCC to %d\n", current_ma);
		...
		struct ilim_entry *ilim)
{
	...
	if (rc) {
		...
		pr_dbg(PR_STATUS, "btm ilim = (%duV %duV %dmA %dmA %dmA)\n",
...
static void smbchg_wipower_icl_deconfigure(struct smbchg_chip *chip)
{
	...
	chip->current_ilim.icl_lv_ma = ilim->icl_lv_ma;
	chip->current_ilim.icl_hv_ma = ilim->icl_hv_ma;
	pr_dbg(PR_WIPOWER, "De config btm\n");
}
...
static void __smbchg_wipower_check(struct smbchg_chip *chip)
{
	...
	if (chg_type != POWER_SUPPLY_CHARGE_TYPE_NONE
		...
		if (rc) {
			pr_dbg(PR_STATUS, "error DCIN read rc = %d\n", rc);
			return;
		}
...
static int smbchg_calc_max_flash_current(struct smbchg_chip *chip)
{
	...
	rc = get_property_from_fg(chip, POWER_SUPPLY_PROP_RESISTANCE,
			&esr_uohm);
	if (rc) {
		pr_dbg(PR_STATUS, "bms psy does not support resistance\n");
		return 0;
	}

	rc = msm_bcl_read(BCL_PARAM_CURRENT, &ibat_now);
	if (rc) {
		pr_dbg(PR_STATUS, "BCL current read failed: %d\n", rc);
		return 0;
	}
...
static int smbchg_iterm_set(struct smbchg_chip *chip, int iterm_ma)
{
	...
	pr_dbg(PR_STATUS, "set tcc (%d) to 0x%02x\n",
			iterm_ma, reg);
				...
				bool flash_active)
{
	...
	if (chip->flash_active == flash_active) {
		pr_dbg(PR_STATUS, "Fsw not changed, flash_active: %d\n",
			flash_active);
		return 0;
	}
				...
				enum skip_reason reason, bool disable)
{
	...
	pr_dbg(PR_STATUS, "%s pulse skip, reason %d\n",
...
static void smbchg_cc_esr_wa_check(struct smbchg_chip *chip)
{
	...
	if (!is_usb_present(chip) && !is_dc_present(chip)) {
		pr_dbg(PR_STATUS, "No inputs present, skipping\n");
		return;
	}
	...
	if (!smbchg_is_input_current_limited(chip)) {
		pr_dbg(PR_STATUS, "Not input current limited, skipping\n");
		return;
	}
	 ...
	 */
	if (esr_count != 0) {
		pr_dbg(PR_STATUS, "ESR count is not zero, skipping\n");
		return;
	}

	pr_dbg(PR_STATUS, "Lowering charge current for ESR pulse\n");
	smbchg_stay_awake(chip, PM_ESR_PULSE);
	smbchg_sw_esr_pulse_en(chip, true);
	msleep(SW_ESR_PULSE_MS);
	pr_dbg(PR_STATUS, "Raising charge current for ESR pulse\n");
...
static void smbchg_aicl_deglitch_wa_en(struct smbchg_chip *chip, bool en)
{
	...
	pr_dbg(PR_STATUS, "AICL deglitch set to %s\n", en ? "short" : "long");
...
static void smbchg_aicl_deglitch_wa_check(struct smbchg_chip *chip)
{
	...
	if (!is_usb_present(chip) && !is_dc_present(chip)) {
		pr_dbg(PR_STATUS, "Charger removed\n");
	...
	if (!low_volt_chgr) {
		pr_dbg(PR_STATUS, "High volt charger! Don't set deglitch\n");
		smbchg_aicl_deglitch_wa_en(chip, false);
		return;
	}
...
static int smbchg_config_chg_battery_type(struct smbchg_chip *chip)
{
	...
	if (!strcmp(chip->battery_name, UNKNOWN_BATT_TYPE) ||
		...
		pr_dbg(PR_MISC, "Battery-type not identified\n");
		return 0;
	}
	...
	if (!batt_node) {
		pr_dbg(PR_MISC, "No batterydata available\n");
		return 0;
	}
	...
	if (rc) {
		pr_dbg(PR_STATUS, "Unable to read battery-type rc=%d\n", rc);
		return 0;
	}
	...
	if (get_pseudo_batt_info(PSEUDO_BATT_MODE)) {
		if ((reg & VBL_MASK) != 0x01) {
			...
			if (rc < 0)
				...
				pr_dbg(PR_LGE, "set vbat low threshold to 2.5V\n");
		}
	} else {
		if ((reg & VBL_MASK) == 0x01) {
			rc = smbchg_sec_masked_write(chip,
			...
			if (rc < 0)
				...
				pr_dbg(PR_LGE, "set vbat low threshold to 0x%x\n",
						chip->vbat_low_thr);
		}
	}
}
...
skip_current_config1:
	...
	if (!chip->lge_cc_lpc) {
		...
		pr_dbg(PR_MISC, "usb type = %s current_limit = %d\n",
...
static int smbchg_otg_regulator_enable(struct regulator_dev *rdev)
{
	...
	pr_dbg(PR_STATUS, "Enabling OTG Boost\n");
...
static int smbchg_otg_regulator_disable(struct regulator_dev *rdev)
{
	...
	pr_dbg(PR_STATUS, "Disabling OTG Boost\n");
	return rc;
}
...
led_brightness smbchg_chg_led_brightness_get(struct led_classdev *cdev)
{
	...
	pr_dbg(PR_STATUS, "chg_led_sts = %02x\n", chg_led_sts);
...
static void smbchg_vfloat_adjust_work(struct work_struct *work)
{
	...
	if ((vbat_mv - chip->vfloat_mv) < -1 * vf_adjust_max_delta_mv) {
		pr_dbg(PR_STATUS, "Skip vbat out of range: %d\n", vbat_mv);
		goto reschedule;
	}
	...
	if (ibat_ua / 1000 > -chip->iterm_ma) {
		pr_dbg(PR_STATUS, "Skip ibat too high: %d\n", ibat_ua);
		goto reschedule;
	}

	pr_dbg(PR_STATUS, "sample number = %d vbat_mv = %d ibat_ua = %d\n",
		chip->n_vbat_samples,
		vbat_mv,
		ibat_ua);
	...
	if (chip->n_vbat_samples < vf_adjust_n_samples) {
		pr_dbg(PR_STATUS, "Skip %d samples; max = %d\n",
	...
	pr_dbg(PR_STATUS, "delta_vfloat_mv = %d, samples = %d, mvbat = %d\n",
						...
						enum power_supply_type type)
{
...
#endif
		pr_dbg(PR_LGE, "factory cable.. set to %dmA\n", FACTORY_ICL_MA);
	...
	pr_dbg(PR_STATUS, "Type %d: setting mA = %d\n",
...
static int smbchg_evp_det_start(struct smbchg_chip *chip, int start)
{
	...
	if (start)
		pr_dbg(PR_LGE, "Start EVP detection\n");
...
static void smbchg_hvdcp_det_prepare_work(struct work_struct *work)
{
	...
	pr_dbg(PR_LGE, "start prepare hvdcp (%d)\n", chip->hvdcp_det_retry);
	...
	if (lge_get_boot_mode() == LGE_BOOT_MODE_QEM_56K ||
		...
		pr_dbg(PR_LGE, "MiniOS boot, enable HVDCP\n");
	...
	if (prop.intval > 5000) {
		pr_dbg(PR_LGE, "HV PD charger detected. ignore HVDCP\n");
		goto out;
	}
	...
	if (chip->hvdcp_uv_recovery) {
		pr_dbg(PR_LGE, "recover under voltage, enable HVDCP\n");
		goto prepare_hvdcp_detection;
	}
#endif
	...
	if (!chip->ctype_rp) {
		if (chip->hvdcp_det_retry >= HVDCP_RETRY_MAX) {
			pr_dbg(PR_LGE, "invalid voltage on CC, ignore HVDCP\n");
			goto out;
		}
	...
	if (chip->ctype_rp == RP_10K) {
		pr_dbg(PR_LGE, "Rp = %dk, ignore HVDCP\n",
					chip->ctype_rp);
		goto out;
	} else
#endif
	if (chip->ctype_rp == RP_NONE) {
		if (chip->hvdcp_det_retry >= HVDCP_RETRY_MAX) {
			pr_dbg(PR_LGE, "Rp = %dk, do not enable HVDCP\n",
						chip->ctype_rp);
			goto out;
		}
		chip->hvdcp_det_retry++;
...
prepare_hvdcp_detection:
	...
	if (usb_supply_type != POWER_SUPPLY_TYPE_USB_DCP) {
		...
		read_usb_type(chip, &usb_type_name, &usb_supply_type);
		if (usb_supply_type != POWER_SUPPLY_TYPE_USB_DCP) {
			pr_dbg(PR_LGE, "final inserted type = %d (%s)\n",
...
static void smbchg_hvdcp_recovery_work(struct work_struct *work)
{
	...
	chip->hvdcp_uv_count++;
	pr_dbg(PR_LGE, "check recover hvdcp (%d). voltage = %dmV\n",
	...
	pr_dbg(PR_LGE, "start recover hvdcp\n");
	...
	if (rc) {
		pr_dbg(PR_LGE, "error during fake insertion removal\n");
		chip->hvdcp_uv_recovery = false;
		goto out;
	}
	...
	read_usb_type(chip, &usb_type_name, &usb_supply_type);
	if (usb_supply_type != POWER_SUPPLY_TYPE_USB_DCP) {
		pr_dbg(PR_LGE, "error during recovery. inserted type = %d (%s)\n",
				usb_supply_type, usb_type_name);
		chip->hvdcp_uv_recovery = false;
	}
out:
...
static void smbchg_vfloat_trim_check_ok(struct smbchg_chip *chip)
{
	...
	if (chip->vfloat_trim_restore_status){
		pr_dbg(PR_STATUS, "VFLOAT_TRIM restored recharge finish. \n");
...
static void smbchg_vfloat_trim_check_work(struct work_struct *work)
{
	...
	if (capacity == FULL_CHARGED_CAPACITY ) {
		pr_dbg(PR_STATUS, "Skip, SOC (%d) is Full. \n", capacity);
		goto out;
	}
	...
	if( (capacity < FULL_CHARGED_CAPACITY) &&
		...
		pr_dbg(PR_STATUS, "Restore VFLOAT_TRIM register.\n");
		pr_dbg(PR_STATUS, "SOC = %d VBAT = %d mV vfloat_trim_reg %x\n",
			capacity, vbat_mv, vfloat_trim_reg);
		goto restore;
	}else {
		pr_dbg(PR_STATUS, "Not restored. SOC %d VBAT %d trim_reg %x\n",
			capacity, vbat_mv, vfloat_trim_reg);
		goto out;
		}

restore:
	pr_dbg(PR_STATUS, "start recover VFLOAT_TRIM & recharging \n");
	...
	pr_dbg(PR_STATUS, "VFlt trim restored. 0x%02x \n",
	...
	if (rc < 0) {
		pr_dbg(PR_STATUS, "error during restart charging.\n");
		goto reschedule;
	}
	...
	if (chip->psy_registered) {
		if (!chip->enable_aicl_wake) {
			pr_dbg(PR_STATUS, "enable aicl_done_irq\n");
			enable_irq_wake(chip->aicl_done_irq);
			chip->enable_aicl_wake = true;
		}
	} else {
		pr_dbg(PR_STATUS, "smbchg irqs are not registered\n");
	}
	smbchg_parallel_usb_check_ok(chip);
	if (chip->psy_registered)
		power_supply_changed(&chip->batt_psy);
...
out:
	pr_dbg(PR_STATUS, "End of vfloat trim check. \n");
	smbchg_relax(chip, PM_VFLOAT_TRIM_RESTORE);
	return;

reschedule:
	pr_dbg(PR_STATUS, "Rescheduled vfloat trim check. \n");
...
static int set_usb_psy_dp_dm(struct smbchg_chip *chip, int state)
{
	...
	if (!rc && !(reg & USBIN_UV_BIT) && !(reg & USBIN_SRC_DET_BIT)) {
		pr_dbg(PR_MISC, "overwriting state = %d with %d\n",
				state, POWER_SUPPLY_DP_DM_DPF_DMF);
		state = POWER_SUPPLY_DP_DM_DPF_DMF;
	}
	pr_dbg(PR_MISC, "setting usb psy dp dm = %d\n", state);
...
static void restore_from_hvdcp_detection(struct smbchg_chip *chip)
{
	...
	if ((chip->schg_version == QPNP_SCHG_LITE)
		...
		pr_dbg(PR_MISC, "Forcing 9V HVDCP 2.0\n");
		rc = force_9v_hvdcp(chip);
		if (rc)
			pr_err("Failed to force 9V HVDCP=%d\n",	rc);
	}

	pr_dbg(PR_MISC, "Retracting HVDCP vote for ICL\n");
...
static int smbchg_restricted_charging(struct smbchg_chip *chip, bool enable)
{
	...
	pr_dbg(PR_STATUS, "restricted_charging set to %d\n", enable);
...
static void weak_batt_pack_check_work(struct work_struct *work)
{
	...
	if (is_usb_present(chip)) {
		...
		pr_dbg(PR_LGE, "weak battery pack is detected, count = %d\n",
				chip->batt_pack_verify_cnt);
		if (chip->batt_pack_verify_cnt >= WEAK_CHG_DET_CNT
				&& is_usb_present(chip)) {
			pr_dbg(PR_LGE, "weak batt pack detected, set ICL to 1A\n");
			rc = vote(chip->usb_icl_votable,
					WEAK_CHARGER_ICL_VOTER, true, DEFAULT_WEAK_ICL_MA);
		}
	} else {
		pr_dbg(PR_LGE, "usb is removed. reset count\n");
...
static void handle_usb_removal(struct smbchg_chip *chip)
{
	...
	if (chip->parallel.use_parallel_aicl) {
		complete_all(&chip->hvdcp_det_done);
		pr_dbg(PR_MISC, "complete hvdcp_det_done\n");
	}

	if (!chip->skip_usb_notification) {
		pr_dbg(PR_MISC, "setting usb psy present = %d\n",
				chip->usb_present);
#ifdef CONFIG_LGE_USB_TYPE_C
		if (!chip->typec_psy) {
			chip->typec_psy = power_supply_get_by_name("usb_pd");
	...
	schedule_work(&chip->usb_set_online_work);
	pr_dbg(PR_MISC, "setting usb psy health UNKNOWN\n");
...
#ifdef CONFIG_LGE_PM_VFLOAT_TRIM_RESTORE
	if (chip->vfloat_trim_restore_status){
		pr_dbg(PR_STATUS, "VFLOAT_TRIM wakelock released. \n");
		chip->vfloat_trim_restore_status = false;
		smbchg_relax(chip, PM_VFLOAT_TRIM_RECHARGE);
	}
#endif
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	if (!chip->usb_ov_det) {
		 ...
		 */
		pr_dbg(PR_MISC, "setting usb psy health %s\n",
				chip->very_weak_charger
				? "UNSPEC_FAILURE" : "GOOD");
		rc = power_supply_set_health_state(chip->usb_psy,
				chip->very_weak_charger
	...
	if (!chip->hvdcp_not_supported &&
		...
		if (chip->parallel.use_parallel_aicl) {
			...
			pr_dbg(PR_MISC, "init hvdcp_det_done\n");
		}
	}
#ifdef CONFIG_LGE_PM_PARALLEL_CHARGING
	schedule_delayed_work(&chip->battchg_protect_work,
	...
	if (chip->input_blocked) {
		pr_dbg(PR_STATUS, "sending waterproof signal for UI update\n");
		power_supply_changed(&chip->batt_psy);
	}
#endif
}
...
static void increment_aicl_count(struct smbchg_chip *chip)
{
	...
	pr_dbg(PR_INTERRUPT, "aicl count c:%d dgltch:%d first:%ld\n",
	...
	if (chip->aicl_deglitch_short || chip->force_aicl_rerun) {
		...
		if (chip->aicl_irq_count > max_aicl_count) {
			pr_dbg(PR_INTERRUPT, "elp:%ld first:%ld now:%ld c=%d\n",
				elapsed_seconds, chip->first_aicl_seconds,
				now_seconds, chip->aicl_irq_count);
			pr_dbg(PR_INTERRUPT, "Disable AICL rerun\n");
...
static int fake_insertion_removal(struct smbchg_chip *chip, bool insertion)
{
	...
	pr_dbg(PR_MISC, "Allow only %s charger\n",
	...
	pr_dbg(PR_MISC, "Waiting on %s usbin uv\n",
	...
	pr_dbg(PR_MISC, "Waiting on %s src det\n",
...
static int smbchg_prepare_for_pulsing(struct smbchg_chip *chip)
{
	...
	/* switch to 5V HVDCP */
	pr_dbg(PR_MISC, "Switch to 5V HVDCP\n");
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + CHGPTH_CFG,
				HVDCP_ADAPTER_SEL_MASK, HVDCP_5V);
	if (rc < 0) {
	 ...
	 */
	if (!is_src_detect_high(chip)) {
		pr_dbg(PR_MISC, "src det low after 500mS sleep\n");
		goto out;
	}

	/* disable HVDCP */
	pr_dbg(PR_MISC, "Disable HVDCP\n");
	...
	pr_dbg(PR_MISC, "HVDCP voting for 300mA ICL\n");
	...
	/* fake an insertion */
	pr_dbg(PR_MISC, "Faking Insertion\n");
	 ...
	 */
	if (!is_src_detect_high(chip)) {
		pr_dbg(PR_MISC, "src det low after 2s sleep\n");
		rc = -EINVAL;
		goto out;
	}
	...
	if ((reg >> TYPE_BITS_OFFSET) != 0) {
		pr_dbg(PR_MISC, "type bits set after 2s sleep - abort\n");
		rc = -EINVAL;
		goto out;
	}
...
static int smbchg_unprepare_for_pulsing(struct smbchg_chip *chip)
{
	...
	/* switch to 9V HVDCP */
	pr_dbg(PR_MISC, "Switch to 9V HVDCP\n");
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + CHGPTH_CFG,
				HVDCP_ADAPTER_SEL_MASK, HVDCP_9V);
	if (rc < 0) {
		pr_err("Couldn't configure HVDCP 9V rc=%d\n", rc);
...
static int rerun_apsd(struct smbchg_chip *chip)
{
	...
	if (chip->schg_version == QPNP_SCHG_LITE) {
		pr_dbg(PR_STATUS, "Re-running APSD\n");
		reinit_completion(&chip->src_det_raised);
		reinit_completion(&chip->usbin_uv_lowered);
		reinit_completion(&chip->src_det_lowered);
		reinit_completion(&chip->usbin_uv_raised);
		...
		pr_dbg(PR_MISC, "Waiting on rising usbin uv\n");
		...
		pr_dbg(PR_MISC, "Waiting on falling src det\n");
		...
		pr_dbg(PR_MISC, "Waiting on falling usbin uv\n");
		...
		pr_dbg(PR_MISC, "Waiting on rising src det\n");
		...
		pr_dbg(PR_STATUS, "Faking Removal\n");
		rc = fake_insertion_removal(chip, false);
		msleep(500);
		pr_dbg(PR_STATUS, "Faking Insertion\n");
		rc = fake_insertion_removal(chip, true);
	}
...
static bool is_hvdcp_5v_cont_mode(struct smbchg_chip *chip)
{
	...
	pr_dbg(PR_STATUS, "HVDCP status = %x\n", reg);
	...
	if (reg & SCHG_LITE_USBIN_HVDCP_SEL_IDLE) {
		...
		pr_dbg(PR_STATUS, "INPUT status = %x\n", reg);
...
static int smbchg_prepare_for_pulsing_lite(struct smbchg_chip *chip)
{
	...
	if (is_hvdcp_5v_cont_mode(chip)) {
		pr_dbg(PR_MISC, "HVDCP by default is in 5V continuous mode\n");
		return 0;
	}
...
out:
	...
	restore_from_hvdcp_detection(chip);
	if (!is_src_detect_high(chip)) {
		pr_dbg(PR_MISC, "HVDCP removed - force removal\n");
...
static int smbchg_dp_dm(struct smbchg_chip *chip, int val)
{
	...
	case POWER_SUPPLY_DP_DM_HVDCP3_SUPPORTED:
		...
		pr_dbg(PR_MISC, "HVDCP3 supported\n");
		break;
...
static int smbchg_get_prop_batt_charge_counter(struct smbchg_chip *chip)
{
	...
	if (rc < 0) {
		pr_dbg(PR_STATUS, "Couldn't get charge count rc = %d\n", rc);
		return rc;
	}
...
static int smbchg_get_prop_batt_current_max(struct smbchg_chip *chip)
{
	...
	if (rc < 0) {
		pr_dbg(PR_STATUS, "Couldn't get current max rc = %d\n", rc);
		return rc;
	}
					...
					const union power_supply_propval *val)
{
	...
	pr_dbg(PR_TYPEC, "typec capability = %dma\n", val->intval);
...
//	chip->cd_current_limit = 0;
	...
	if (is_factory_cable(chip) && chip->usb_present) {
		...
		lgcc_set_ibat_current(chip, chip->otp_ibat_current);
		pr_dbg(PR_LGE, "factory cable.. skipping setting current\n");
	...
	if (chip->lge_cd_lpc) {
		...
		if (rc == 0 && lge_val.intval == 1) {
			...
			if (rc == 0) {
				current_limit = lge_val.intval / 1000;
				pr_dbg(PR_LGE, "usb current max. setting current %d\n",
						current_limit);
			}
		}
	}
	...
	if (!chip->usb_psy || !chip->usb_psy->get_property ||
		...
		pr_dbg(PR_LGE, "failed to get apsd_rerun_need\n");
		prop.intval = 0;
	}

	if (chip->typec_psy && chip->typec_current_ma && !prop.intval) {
		pr_dbg(PR_LGE, "skip_current_for_non_sdp, typec_current_ma(%d), apsd_rerun_need(%d)\n",
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_RERUN_AICL:
		 ...
		 */
		pr_dbg(PR_LGE, "rerun aicl triggered in userspace.\n");
		chip->aicl_irq_count = 0;
#endif
		smbchg_rerun_aicl(chip);
		break;
	...
	case POWER_SUPPLY_PROP_CTYPE_RP:
		...
		if (chip->ctype_rp != RP_NONE && val->intval != RP_NONE) {
			pr_dbg(PR_LGE, "Ignore rp = %d\n", val->intval);
			break;
		}
#endif
		chip->ctype_rp = val->intval;
		pr_dbg(PR_LGE, "usb typec rp check = %d\n", chip->ctype_rp);
		break;
#endif
#ifdef CONFIG_LGE_PM_CHARGING_SCENARIO
#ifdef CONFIG_MACH_MSM8996_LUCYE
	...
	case POWER_SUPPLY_PROP_INPUT_SUSPEND :
		if (chip->input_blocked != !!val->intval) {
			...
			pr_dbg(PR_LGE, "waterproof : USBIN is %s.\n", chip->input_blocked ? "blocked" : "released");
		}
		break;
#endif
	default:
...
static irqreturn_t batt_hot_handler(int irq, void *_chip)
{
	...
	pr_dbg(PR_INTERRUPT, "triggered: 0x%02x\n", reg);
	smbchg_parallel_usb_check_ok(chip);
	if (chip->psy_registered)
		power_supply_changed(&chip->batt_psy);
	smbchg_charging_status_change(chip);
...
static irqreturn_t chg_error_handler(int irq, void *_chip)
{
	...
	pr_dbg(PR_INTERRUPT, "chg-error triggered\n");
...
static irqreturn_t fastchg_handler(int irq, void *_chip)
{
	...
	pr_dbg(PR_INTERRUPT, "p2f triggered\n");
#ifdef CONFIG_LGE_PM
	if (!chip->usb_present) {
		pr_dbg(PR_LGE, "illegal interrupt triggered, ignore\n");
		return IRQ_HANDLED;
	}
#endif
...
static irqreturn_t chg_term_handler(int irq, void *_chip)
{
	...
	int rc;
#endif
	pr_dbg(PR_INTERRUPT, "tcc triggered\n");
	/*
	...
	if (chip->psy_registered) {
		if (chip->enable_aicl_wake) {
			pr_dbg(PR_LGE, "disable aicl_done_irq\n");
			disable_irq_wake(chip->aicl_done_irq);
			chip->enable_aicl_wake = false;
		}
	} else {
		pr_dbg(PR_LGE, "smbchg irqs are not registered\n");
	}
#endif
...
static irqreturn_t recharge_handler(int irq, void *_chip)
{
	...
	if (chip->psy_registered) {
		if (!chip->enable_aicl_wake) {
			pr_dbg(PR_LGE, "enable aicl_done_irq\n");
			enable_irq_wake(chip->aicl_done_irq);
			chip->enable_aicl_wake = true;
		}
	} else {
...
static irqreturn_t dcin_uv_handler(int irq, void *_chip)
{
	...
	pr_dbg(PR_STATUS, "chip->dc_present = %d dc_present = %d\n",
...
static irqreturn_t usbin_ov_handler(int irq, void *_chip)
{
	...
	if (reg & USBIN_OV_BIT) {
		...
		if (chip->usb_psy) {
			pr_dbg(PR_MISC, "setting usb psy health OV\n");
			rc = power_supply_set_health_state(chip->usb_psy,
...
static void increment_uv_irq_count(struct smbchg_chip *chip)
{
	...
	chip->uv_irq_count++;
	pr_dbg(PR_LGE, "uv_irq_count[%d]\n", chip->uv_irq_count);

	if (chip->uv_irq_count >= 10) {
		prop.intval = CURRENT_100_MA * 1000;
		pr_dbg(PR_LGE, "icl set 100 mA by weak VBUS\n");
		rc = chip->usb_psy->set_property(chip->usb_psy,
				POWER_SUPPLY_PROP_CURRENT_MAX, &prop);
		if (rc < 0) {
		...
		pr_dbg(PR_LGE, "usb suspend by weak charger\n");
...
static irqreturn_t usbin_uv_handler(int irq, void *_chip)
{
	...
	if (!(reg & USBIN_UV_BIT) && !(reg & USBIN_SRC_DET_BIT) &&
		!chip->hvdcp_3_det_ignore_uv) {
		pr_dbg(PR_MISC, "setting usb psy dp=f dm=f\n");
	...
	if ((reg & USBIN_UV_BIT) && (reg & USBIN_SRC_DET_BIT)) {
#ifdef CONFIG_LGE_PM_DEBUG
		pr_dbg(PR_STATUS, "Very weak charger detected. VBUS[%d]\n",
				get_usb_adc(chip));
#else
		pr_dbg(PR_STATUS, "Very weak charger detected\n");
#endif
		chip->very_weak_charger = true;
		rc = smbchg_read(chip, &reg,
				chip->usb_chgpth_base + ICL_STS_2_REG, 1);
		...
		if (!chip->lge_cc_lpc)
			...
			if (rc < 0)
				pr_dbg(PR_LGE, "could not get current_max mode,"
					" rc=%d\n", rc);
			else
		...
		pr_dbg(PR_LGE, "usb_type[%s], current_max_mode[%d]\n",
			usb_type_name,  current_max_mode);

		if (usb_supply_type == POWER_SUPPLY_TYPE_USB ||
			usb_supply_type == POWER_SUPPLY_TYPE_USB_CDP) {
			if (current_max_mode) {
				pr_dbg(PR_LGE, "skip usb_suspend votable\n");
				goto out;
			}
		...
		pr_dbg(PR_MISC, "setting usb psy health UNSPEC_FAILURE\n");
		rc = power_supply_set_health_state(chip->usb_psy,
				POWER_SUPPLY_HEALTH_UNSPEC_FAILURE);
		if (rc)
			pr_err("Couldn't set health on usb psy rc:%d\n", rc);
...
static irqreturn_t otg_fail_handler(int irq, void *_chip)
{
	...
	if (chip->bms_psy) {
		...
		if (rc < 0)
			pr_dbg(PR_LGE, "failed to read Battery Volatge\n");
		else
			pr_dbg(PR_LGE, "vbat : %d\n", ret.intval / 1000);
	} else {
		pr_dbg(PR_LGE, "bms_psy is not ready yet\n");
	}
#endif
	pr_dbg(PR_INTERRUPT, "triggered\n");
	return IRQ_HANDLED;
...
static irqreturn_t aicl_done_handler(int irq, void *_chip)
{
	...
	pr_dbg(PR_INTERRUPT, "triggered, aicl: %d\n", aicl_level);
...
static irqreturn_t usbid_change_handler(int irq, void *_chip)
{
	...
	if (chip->usb_psy) {
		pr_dbg(PR_MISC, "setting usb psy OTG = %d\n",
				otg_present ? 1 : 0);
		power_supply_set_usb_otg(chip->usb_psy, otg_present ? 1 : 0);
	}
	if (otg_present)
		pr_dbg(PR_STATUS, "OTG detected\n");
...
static void lgcc_charger_reginfo(struct work_struct *work) {
	...
	if (!chip->bms_psy || !chip->usb_psy || !parallel_psy) {
		pr_dbg(PR_LGE, "failed to get power_supply\n");
		return;
	}
#ifdef CONFIG_LGE_PM_LGE_POWER_CLASS_ADC_QCT
	if (!lge_adc_lpc)
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read Parallel STATUS\n");
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read PMI ICL\n");
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read SMB IBAT\n");
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read Battery SOC\n");
	batt_soc = ret.intval;

	rc = chip->usb_psy->get_property(chip->usb_psy,
		POWER_SUPPLY_PROP_CURRENT_MAX, &ret);
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read TOTAL IUSB\n");
	total_iusb_set = ret.intval / 1000;

	rc = chip->batt_psy.get_property(&chip->batt_psy,
		POWER_SUPPLY_PROP_CURRENT_NOW, &ret);
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read Total IBAT NOW\n");
	batt_cur = ret.intval;
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read Battery temperature\n");
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	pr_dbg(PR_STATUS, "Charger Revision DIG: %d.%d; ANA: %d.%d\n",
	...
	if (chip->vfloat_mv != -EINVAL) {
		...
		pr_dbg(PR_STATUS, "set vfloat to %d\n", chip->vfloat_mv);
	}
	...
	if (chip->fastchg_current_comp != -EINVAL) {
		...
		pr_dbg(PR_STATUS, "set fastchg current comp to %d\n",
			chip->fastchg_current_comp);
	}
	...
	if (chip->float_voltage_comp != -EINVAL) {
		...
		pr_dbg(PR_STATUS, "set float voltage comp to %d\n",
			chip->float_voltage_comp);
	}
	...
	chip->vfloat_trim_restore_status = false;
	pr_dbg(PR_STATUS, "Initial VFLOAT_TRIM reg = 0x%x\n",
		chip->initial_vfloat_trim_reg);
#endif
...
static int batt_life_cycle_dt_init (struct smbchg_chip* chip) {
    ...
    for (i = 0; i < size; i++) {
        ...
        pr_dbg(PR_LGE, "batt_life_cycle_fcc_ma %d : %d\n",
    ...
    for (i = 0; i < size; i++) {
        ...
        pr_dbg(PR_LGE, "batt_life_cycle_vfloat %d : %d\n",
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	of_property_read_u32(chip->spmi->dev.of_node,
					...
					&smbchg_main_chg_icl_percent);
	pr_dbg(PR_STATUS, "parallel usb thr: %d, 9v thr: %d\n",
...
static int smbchg_check_chg_version(struct smbchg_chip *chip)
{
	...
	pr_dbg(PR_STATUS, "pmic=%s, wa_flags=0x%x, hvdcp_supported=%s\n",
...
static void rerun_hvdcp_det_if_necessary(struct smbchg_chip *chip)
{
	...
	if (usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP
		&& !is_hvdcp_present(chip)) {
		pr_dbg(PR_STATUS, "DCP found rerunning APSD\n");
#ifdef CONFIG_LGE_PM
		...
		if (usb_supply_type != POWER_SUPPLY_TYPE_USB_DCP) {
			msleep(500);
			pr_dbg(PR_STATUS, "Rerun APSD as type !=DCP\n");
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	if (!usb_psy) {
		pr_dbg(PR_STATUS, "USB supply not found, deferring probe\n");
		return -EPROBE_DEFER;
	}
...
#ifdef CONFIG_LGE_PM_WATERPROOF_PROTECTION
{
	...
	if (psy_usb_pd && psy_usb_pd->get_property &&
		...
		pr_dbg(PR_LGE, "The initial value of input_blocked = %d\n",
			chip->input_blocked);
	}
	else {
		pr_dbg(PR_LGE, "usb_pd psy is not ready...\n");
	}
...
#endif
#ifdef CONFIG_LGE_PM
	pr_dbg(PR_LGE, "smbchg_probe end\n");
#endif
...
static void smbchg_shutdown(struct spmi_device *spmi)
{
	...
	pr_dbg(PR_MISC, "Wait 500mS to lower to 5V\n");
	/* wait for HVDCP to lower to 5V */
	msleep(500);
	/*
	 * Check if the same hvdcp session is in progress. src_det should be
	...
	pr_dbg(PR_MISC, "Wait 1S to settle\n");
	msleep(1000);
	chip->hvdcp_3_det_ignore_uv = false;

	pr_dbg(PR_STATUS, "wrote power off configurations\n");
}
