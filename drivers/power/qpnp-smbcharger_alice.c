	...
	int total_iusb)
{
	...
	if (i < 0) {
		pr_dbg(PR_LGE, "can not find match table. "
...
int lgcc_is_charger_present(void)
{
	...
	pr_dbg(PR_LGE, "charger_present[%d]\n", charger_present);
...
{
	if (on) {
		pr_dbg(PR_LGE, "disable parallel charger\n");
		smbchg_parallel_usb_disable(smb_chip);
	} else {
		pr_dbg(PR_LGE, "enable parallel charger\n");
	}
}
...
int lgcc_set_ibat_current(int type, int state, int chg_current)
{
	...
	pr_dbg(PR_LGE, "type[%d], state[%d], chg_current[%d]\n",
...
int lgcc_set_iusb_enable(int enable, int state)
{
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to set IUSB Status\n");
...
int lgcc_get_effective_fcc_result(void)
{
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to get fcc votable\n");
...
static void lgcc_charger_reginfo(struct work_struct *work)
{
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read PMI IBAT\n");
	...
	pr_dbg(PR_LGE, "[C], USB_PRESENT, WIRELESS_PRESENT, PARALLEL_STATUS, "
...
#endif
		pr_dbg(PR_LGE, "factory cable - set usb_max_current_ma to %dmA\n",
				FACTORY_ICL_MA);
...
static void smbchg_parallel_usb_disable(struct smbchg_chip *chip)
{
	...
	{
		pr_dbg(PR_LGE, "parallel charger is disabled. set IBAT to 1000mA\n");
		...
		int smb_ibat)
{
	...
	pr_dbg(PR_LGE, "dual ibat pmi = %d, smb = %d, total = %d\n",
...
try_again:
	...
	pr_dbg(PR_STATUS, "reducing FCC to %dmA\n", dual_taper_current.total);
		...
		int total_current_ma)
{
	...
	if (lge_get_board_revno() == HW_REV_0_1) {
		pr_dbg(PR_LGE, "disable parallel charging at Rev0_1\n");
		if (chip->parallel.current_max_ma != 0) {
			pr_dbg(PR_STATUS, "disabling parallel charger\n");
			smbchg_parallel_usb_disable(chip);
		}
	...
	if (skip_fcc_set &&
		...
		pr_dbg(PR_LGE, "TOTAL IUSB[%d] - PMI IUSB[%d], SMB IUSB[%d]\n",
	...
	if (rc < 0)
		pr_dbg(PR_LGE, "failed to read Charger Type\n");

	if (pval.intval == POWER_SUPPLY_TYPE_USB_DCP) {
		pr_dbg(PR_LGE, "DCP inserted. skip AICL recovery\n");
...
#ifdef CONFIG_LGE_PM_PARALLEL_CHARGING
	if (chip->is_parallel_chg_dis_by_taper) {
		pr_dbg(PR_LGE, "disable parallel charging by taper handler\n");
		return false;
	}
#endif
...
#ifdef CONFIG_LGE_PM_PARALLEL_CHARGING
	pr_dbg(PR_LGE, "client[%d], suspend[%d], last_client[%d], last_suspend[%d]\n",
...
static void smbchg_usb_pd_en(struct smbchg_chip *chip)
{
	...
	if (c_type == POWER_SUPPLY_TYPE_CTYPE ||
		...
		if (c_ma == 0 || c_ma == target_icl_ma || target_icl_ma == MAX_ICL_MA) {
			pr_dbg(PR_MISC, "skip current setting for C Type\n");
		} else {
			pr_dbg(PR_LGE, "Type[%d], c_mV[%d], c_mA[%d], "
			...
			if (rc < 0)
				pr_dbg(PR_LGE, "Couldn't vote for "
						"new USB ICL rc=%d\n", rc);
		}
	}
}
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	if (smbchg_is_hvdcp_type(chip) && get_usb_adc() < HVDCP_UV_MV) {
		...
		if (prop.intval == 1) {
			pr_dbg(PR_LGE, "rerun apsd by vbus drop\n");
...
restore:
	...
	if (rc < 0) {
		pr_dbg(PR_STATUS, "error during suspend charging.\n");
		goto reschedule;
	}
...
static void smbchg_hvdcp_check_work(struct work_struct *work)
{
	...
	if (smbchg_is_hvdcp_type(chip) && (vbus_mv < HVDCP_UV_MV)) {
	    pr_dbg(PR_LGE, "rerun apsd is needed, vbus[%d]\n", vbus_mv);
		prop.intval = 1;
...
static void smbchg_evp_check_work(struct work_struct *work)
{
	...
	pr_dbg(PR_LGE, "is_charger_type[%d], PD_EN[%d]\n", pval.intval, c_type);

	if (c_type != POWER_SUPPLY_TYPE_CTYPE &&
			c_type != POWER_SUPPLY_TYPE_CTYPE_PD &&
			pval.intval != POWER_SUPPLY_TYPE_USB_HVDCP &&
			pval.intval != POWER_SUPPLY_TYPE_USB_HVDCP_3) {
		pr_dbg(PR_LGE, "usb type is DCP. Start EVP detection\n");
		pval.intval = 1;
		chip->usb_psy->set_property(chip->usb_psy,
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	if (!chip->hvdcp_not_supported &&
		...
		pr_dbg(PR_LGE, "hvdcp_check_work start\n");
#endif
	}
...
static void lgcc_work_enable_check(struct work_struct *work)
{
	...
	if (is_usb_present(chip)) {
		...
		if (lgcc_is_probed != 1) {
			pr_dbg(PR_LGE, "lgcc is not probed yet. "
...
#ifdef CONFIG_LGE_PM_CHARGING_CONTROLLER
		if (lgcc_is_probed == 1) {
			pr_dbg(PR_LGE, "usb insertion. start lgcc_work\n");
			start_lgcc_work(LGCC_WORK_ENABLE_DELAY);
...
#ifdef CONFIG_LGE_PM_CHARGING_CONTROLLER
		if (lgcc_is_probed == 1) {
			pr_dbg(PR_LGE, "usb removal. stop battemp_work\n");
			stop_lgcc_work();
		}
#endif
#ifdef CONFIG_LGE_PM_WAKE_LOCK_FOR_CHG_LOGO
...
static void smbchg_rerun_hvdcp_work(struct work_struct *work)
{
	...
	if (!chip->hvdcp_not_supported &&
		...
		pr_dbg(PR_LGE, "evp_check_work start\n");
#endif
...
static irqreturn_t chg_term_handler(int irq, void *_chip)
{
	...
	if (!(reg & BAT_TCC_REACHED_RT_STS) &&
		...
		pr_dbg(PR_LGE, "Recharging triggered\n");
		chip->is_parallel_chg_dis_by_taper = false;
	}
#endif
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	chip->vfloat_trim_restore_status = false;
	pr_dbg(PR_STATUS, "VFLOAT_TRIM reg during booting = 0x%x\n",
			chip->initial_vfloat_trim_reg);
#endif
	rc = configure_icl_control(chip, ICL_BUF_SYSON_LDO_VAL);
	if (rc)
...
#ifdef CONFIG_LGE_PM_CHARGING_CONTROLLER
	pr_dbg(PR_LGE, "smbchg_probe start for alice\n");
#endif
